...
if ($type == "add") {
    ...
    foreach ($book_image as $value) {
        $sql_tmp = "INSERT INTO book_image (book_id, image_id) VALUES ('$book_id', '$value')";
        if (!(mysqli_query($connect, $sql_tmp))) {
            $cerror++;
            echo "Error inserting image : " . mysqli_error($connect);
            exit;
        }
    }
    ...
    $sql_tmp = "UPDATE book SET book_name = '$book_name', book_chapter = '$book_chapter', book_isbn = '$book_isbn', book_number_of_pages = '$book_npage', book_year = '$book_year', book_price = '$book_price', book_edition = '$book_edition', book_language = '$book_language', book_summary = '$book_summary', publisher_id = '$publisher_id' WHERE book_id = '$book_id'";
    if (!(mysqli_query($connect, $sql_tmp))) {
        echo "Error updating 1 : " . mysqli_error($connect);
        exit;
    }
    $sql_tmp = "DELETE FROM book_written WHERE book_id = '$book_id'";
    if (!(mysqli_query($connect, $sql_tmp))) {
        echo "Error updating 2 : " . mysqli_error($connect);
        exit;
    }

    $sql_tmp = "DELETE FROM book_translated WHERE book_id = '$book_id'";
    if (!(mysqli_query($connect, $sql_tmp))) {
        echo "Error updating 3 : " . mysqli_error($connect);
        exit;
    }
    ...
    if ($cerror == 0) {
        echo "edite successful";
        echo "<script>window.close()</script>";
    }
} elseif ($type == "delete_book") {
    $book_id = $_POST["book_id"];

    $sql_tmp = "DELETE FROM book WHERE book_id = '$book_id'";
    if (!(mysqli_query($connect, $sql_tmp))) {
...
} else if ($type == "delete_interpreter") {
    $interpreter_id = $_POST["interpreter_id"];
    $sql = "DELETE FROM interpreter WHERE interpreter_id = '$interpreter_id'";
    if (!(mysqli_query($connect, $sql))) {
        $cerror++;
        echo "Error edition : " . mysqli_error($connect);
    } else {
    ...
    $result = mysqli_query($connect, $sql);
    if (!mysqli_fetch_assoc($result)) {
        echo $borrower_birthday." ".$borrower_register_date;
        $sql = "INSERT INTO borrower (borrower_id, borrower_firstname, borrower_lastname, borrower_register_date,borrower_phone, borrower_email) 
        VALUES ('$borrower_id', '$borrower_firstname', '$borrower_lastname', '$borrower_register_date','$borrower_phone', '$borrower_email')";
        if (!(mysqli_query($connect, $sql))) {
            $cerror++;
            echo "Error adding borrower : " . mysqli_error($connect);
        } else {
            echo "add successful";
            echo "<script>window.close()</script>";
        }
    } else {
        echo "borrower is exist!";
        echo "<script>window.close()</script>";
    }
}
echo "eh";
